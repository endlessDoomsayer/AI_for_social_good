...
def temporal_decomposition(self, time_windows):
    """Divide problem into time slices"""
    subproblems = []
    for window_start, window_end in time_windows:
        subproblem = {
            'jobs': [(i, j) for i in self.data["I"] for j in self.data["J"]
                    if j <= self.data["n_jobs"][i] and
                    self.data["THRESHOLD_FOR_JOB_J_AND_I"].get((i, j), self.data["T_MAX"]) >= window_start],
            'time_range': (window_start, window_end),
            'energy_capacity': self.data["p"][window_start:window_end+1]
        }
...
def machine_decomposition(self):
    ...
    for i in self.data["I"]:
        ...
        if energy_category not in machine_groups:
            machine_groups[energy_category] = []
        machine_groups[energy_category].append(i)

    return [{'machines': machines, 'jobs': [(i, j) for i in machines
                                            for j in self.data["J"] if j <= self.data["n_jobs"][i]]}
            for machines in machine_groups.values()]


def tree_decomposition(self):
    """Create tree decomposition based on job dependencies"""
    # Level 1: Critical path jobs (highest priority)
    critical_jobs = self._identify_critical_jobs()

    # Level 2: Jobs with dependencies on critical jobs
    dependent_jobs = self._find_dependent_jobs(critical_jobs)

    # Level 3: Independent jobs that can be freely scheduled
    independent_jobs = self._find_independent_jobs()

    return {
        'critical': critical_jobs,
        'dependent': dependent_jobs,
        'independent': independent_jobs
    }


def _identify_critical_jobs(self):
    """Identify jobs on critical path or with tight constraints"""
    critical = []
    for i in self.data["I"]:
        for j in self.data["J"]:
            if j <= self.data["n_jobs"][i]:
                # Job is critical if it has tight time windows or high energy requirements
                threshold = self.data["THRESHOLD_FOR_JOB_J_AND_I"].get((i, j), self.data["T_MAX"])
                if threshold - self.data["d"][i] < 5:  # Tight time window
                    critical.append((i, j))
    return critical


def energy_based_decomposition(self):
    """Decompose based on energy consumption patterns"""
    high_energy_jobs = []
    medium_energy_jobs = []
    low_energy_jobs = []

    energy_threshold_high = max(self.data["e"]) * 0.7
    energy_threshold_low = max(self.data["e"]) * 0.3

    for i in self.data["I"]:
        for j in self.data["J"]:
            if j <= self.data["n_jobs"][i]:
                total_energy = self.data["e"][i] + self.data["f"][i]
                if total_energy > energy_threshold_high:
                    high_energy_jobs.append((i, j))
                elif total_energy > energy_threshold_low:
                    medium_energy_jobs.append((i, j))
                else:
                    low_energy_jobs.append((i, j))

    return {
        'high_energy': high_energy_jobs,
        'medium_energy': medium_energy_jobs,
        'low_energy': low_energy_jobs
    }


def solve_with_decomposition(self, M_val, N_val):
    ...
    # Step 3: Schedule dependent jobs considering critical job constraints
    remaining_capacity = self._calculate_remaining_capacity(critical_solution, M_val, N_val)
    dependent_solution = self._solve_subproblem(tree_decomp['dependent'],
                                                remaining_capacity['M'], remaining_capacity['N'])

    # Step 4: Fill remaining slots with independent jobs
    final_capacity = self._calculate_remaining_capacity({**critical_solution, **dependent_solution},
                                                        M_val, N_val)
    independent_solution = self._solve_subproblem(tree_decomp['independent'],
                                                  final_capacity['M'], final_capacity['N'])

    # Combine solutions
    complete_solution = {**critical_solution, **dependent_solution, **independent_solution}

    return self._validate_complete_solution(complete_solution, M_val, N_val)

# TODO: soft constraints if you want
