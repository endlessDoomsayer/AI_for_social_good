...
def get_solar_production(data, day, end_date):
...
def generate_data(data, number_of_days):
    ...
    mmm = {t: random.randint(800000, 1000000) for t in data["T"]}
    data["mmm"] = mmm
    ...
    c = {i: (i - 1) // 2 for i in data["I"]}
    data["c"] = c

    # THRESHOLD_FOR_JOB_J
    THRESHOLD_FOR_JOB_J_AND_I = {(i, j): 24 * (j) for i in data["I"] for j in data["J"]}  # Time limit for each job TODO
...
def get_data(number_of_days = 1, day = pd.Timestamp("2018-02-19")):
    ...
    T_MAX = 24*number_of_days-1  # Number of time periods (e.g., 48 half-hours in a day)
...
def get_modified_data(number_of_days=1, day=pd.Timestamp("2018-02-19")):
    end_date = day + pd.to_timedelta(number_of_days - 1, unit='D')
...
def get_data_house(number_of_days = 7, day = pd.Timestamp("2018-02-19")):
