...
# HERE WE DON'T USE AN OBJECTIVE FUNCTION, BUT ONLY CONSTRAINTS
M = 15
N = 20
...
def energy_balance_rule(m, t):
    return sum(e[i] * m.x[i, t, j] + f[i] * m.y[i, t, j] for i in m.I for j in m.J) - M * p[t] - m.s[t] <= m.z[t]
model.energy_balance = pyo.Constraint(model.T, rule=energy_balance_rule)


# 2. Volume calculation constraint
def volume_rule(m, t):
    return m.V[t] == sum(M * p[tp] - sum(e[i] * m.x[i, tp, j] + f[i] * m.y[i, tp, j] for i in m.I for j in m.J)
                         for tp in range(1, t))
