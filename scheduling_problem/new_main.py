...
model_1_M, model_1_N = 0, 0
for policy, (M, N) in number_of_M_N_per_policy.items():
    model_1_M, model_1_N = max(model_1_M, M), max(model_1_N, N)
...
model_3_cost, model_3_M, model_3_N = 0, 0
for policy, (cost, M, N) in number_of_cost_M_N_per_policy.items():
    model_3_cost, model_3_M, model_3_N = max(model_3_cost, cost), max(model_3_M, M), max(model_3_N, N)


# Run Model 4
for policy, (M, N) in MODEL_4_1_POLICIES.items():
    MODEL_4_1_POLICIES[policy] = (model_3_M, model_3_N)
