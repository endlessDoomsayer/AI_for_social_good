...
# Dovrei salvare la data massima 

def run_models_1(policies, days=7, dates = ["2018-01-01", "2018-01-01", "2018-01-01", "2018-01-01"]):
    

    output_file = f"results_model_1.txt"
    number_of_M_N_date_per_policy = {}  
    ...
    with open(output_file, "w") as f:
        ...
        for policy in policies:
            ...
            for date in dates:
                ...
                if policy not in number_of_M_N_date_per_policy:
                    number_of_M_N_date_per_policy[policy] = (0, 0, date)

                old_M, old_N = number_of_M_N_date_per_policy[policy]
                number_of_M_N_date_per_policy[policy] = (max(old_M, M), max(old_N, N), date)

        print(f"\n----------------------------------- MODEL 1 - Summary -----------------------------------\n")
        f.write(f"\n----------------------------------- MODEL 1 - Summary -----------------------------------\n")
        for policy, (M, N) in number_of_M_N_date_per_policy.items():
            print(f"{policy}: M={M}, N={N}, date={date}")
            f.write(f"{policy}: M={M}, N={N}, date={date}\n")

    print(f"\noutput_file_saved '{output_file}'")
    return number_of_M_N_date_per_policy
...
def run_models_3(policies, tot_number_of_days,  days=7, dates = ["2018-01-01", "2018-01-01", "2018-01-01", "2018-01-01"]):
    

    output_file = f"results_model_3.txt"
    number_of_cost_M_N_date_per_policy = {}  
        ...
        for policy in policies:
            ...
            for date in dates:
                ...
                if policy == '_3_local_search':
                    cost, M, N = _3_local_search.solve()
                ...
                if policy not in number_of_cost_M_N_date_per_policy:
                    number_of_cost_M_N_date_per_policy[policy] = (0, 0, 0, date)

                old_cost, old_M, old_N = number_of_cost_M_N_date_per_policy[policy]
                number_of_cost_M_N_date_per_policy[policy] = (max(old_cost, cost), max(old_M, M), max(old_N, N), date)

        print(f"\n----------------------------------- MODEL 1 - Summary -----------------------------------\n")
        f.write(f"\n--------------------days--------------- MODEL 1 - Summary -----------------------------------\n")
        for policy, (M, N) in number_of_cost_M_N_date_per_policy.items():
            print(f"{policy}: cost={cost}, M={M}, N={N}, date={date}")
            f.write(f"{policy}: cost={cost}, M={M}, N={N}\n", date={date})

    print(f"\noutput_file_saved '{output_file}'")
    return number_of_cost_M_N_date_per_policy
